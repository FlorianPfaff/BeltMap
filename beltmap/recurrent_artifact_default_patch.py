...
def _unwrap_patched_callable(func: Any) -> Any:
    """Return the original builder behind this compatibility patch."""
...
def build_recurrent_artifact_map_with_valid_default(
    ...
    *,
    ...
    """Build a recurrence map with the least restrictive valid default.

    ``RecurrentArtifactConfig.min_revolutions == 0`` is the runtime sentinel for
    disabling recurrent-artifact filtering. It is therefore intentionally invalid
    for a function whose sole purpose is to build that map. The public builder,
    however, documents ``config`` as optional. When it is omitted, use one observed
    revolution as the minimum valid construction threshold instead of instantiating
    the disabled sentinel and immediately failing validation.
    """
