...
def _exact_frame_index(value: Any, *, row_number: int) -> int:
    """Parse one finite integer-valued frame identifier without truncation."""
...
def _finite_phase_px(value: Any, *, frame_index: int, row_number: int) -> float:
    """Parse one finite phase estimate used by the dense recurrence series."""
...
def load_unique_phase_px_by_frame(path: Path, frame_count: int) -> list[float]:
    """Reject ambiguous or malformed phase rows before loading the dense series."""
