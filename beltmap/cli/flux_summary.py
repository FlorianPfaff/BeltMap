...
def parse_nonnegative_frame_rate(value: str) -> float:
    try:
        parsed = float(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "frame rate must be a finite non-negative number"
        ) from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Summarize particle flux from BeltMap velocity rows."
    )
    ...
    parser.add_argument(
        "--frame-rate-hz",
        type=parse_nonnegative_frame_rate,
        default=0.0,
    )
    ...
    artifacts = write_science_exports(
        args.output_dir,
        rows,
