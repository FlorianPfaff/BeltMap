...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Snapshot new image files for a future streaming BeltMap driver."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    new_paths = discover_new_stream_frames(
        image_dir,
        state,
        max_new=None if args.max_new == 0 else args.max_new,
    )
        ...
        json.dumps(
            {
                ...
                "last_scan_unix_s": state.last_scan_unix_s,
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    print(
        json.dumps(
            {
                "new_frames": [str(path) for path in new_paths],
