...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--detections-path", type=Path, help="Detection CSV output path."
    )
    parser.add_argument(
        "--detections-per-frame-path", type=Path, help="Per-frame CSV output path."
    )
    parser.add_argument(
        "--tracks-path", type=Path, help="Track membership CSV output path."
    )
    parser.add_argument(
        "--velocities-path", type=Path, help="Velocity CSV output path."
    )
    parser.add_argument(
        "--track-scores-path", type=Path, help="Track-score CSV output path."
    )

    parser.add_argument(
        "--threshold", type=float, help="Detection threshold in high-pass z units."
    )
    parser.add_argument(
        "--mode",
        "--detection-mode",
    ...
    parser.add_argument(
        "--min-area-px", type=int, help="Minimum connected-component area."
    )
    parser.add_argument(
        "--max-area-px",
        type=int,
        help="Optional maximum connected-component area. 0 disables.",
    )
    parser.add_argument(
        "--min-bbox-width-px",
        type=int,
        help="Optional minimum bounding-box width. 0 disables.",
    )
    parser.add_argument(
        "--min-bbox-height-px",
        type=int,
        help="Optional minimum bounding-box height. 0 disables.",
    )
    parser.add_argument(
        "--max-bbox-aspect-ratio",
        type=float,
        help="Optional maximum bounding-box aspect ratio. 0 disables.",
    )
    parser.add_argument(
        "--min-bbox-extent",
        type=float,
        help="Optional minimum area/bbox-area extent. 0 disables.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        help="Projection-valley gap for splitting.",
    )
    parser.add_argument(
        "--split-min-component-area-px",
        type=int,
        help="Minimum area for split pieces. 0 inherits min_area_px.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional Gaussian z-noise added to pseudo-residual frames.",
    )
    parser.add_argument(
        "--random-seed",
        type=int,
        default=0,
        help="Random seed for --noise-sigma. Default: 0",
    )

    parser.add_argument(
        "--crop-height-px",
        type=int,
        help="Rendered crop height. Defaults to BELT_REGION height, then map height.",
    )
    parser.add_argument(
        "--frame-count",
        type=int,
    ...
    parser.add_argument(
        "--period-px",
        type=float,
        help="Belt period for phase unwrapping. Defaults to map height.",
    )

    parser.add_argument(
        "--max-match-distance-px",
        type=float,
        help="PyRecEst tracking match distance. Default derives from belt speed.",
    )
    parser.add_argument(
        "--tracking-max-frame-gap",
        type=float,
        help="Maximum selected-frame gap for track association.",
    )
    parser.add_argument(
        "--min-track-length", type=int, help="Minimum detections per velocity row."
    )
    parser.add_argument(
        "--velocity-fit-method",
        choices=["linear", "theil_sen"],
        help="Velocity slope estimator.",
    )
    parser.add_argument(
        "--track-filter-min-length",
        type=int,
        help="Minimum detections per accepted filtered track.",
    )
    parser.add_argument(
        "--track-filter-min-velocity-ratio-y",
        type=float,
        help="Minimum accepted vertical velocity ratio.",
    )
    parser.add_argument(
        "--track-filter-max-velocity-ratio-y",
        type=float,
        help="Maximum accepted vertical velocity ratio.",
    )
    parser.add_argument(
        "--track-filter-max-abs-x-velocity-px-per-frame",
        type=float,
        help="Optional lateral-velocity gate. 0 disables.",
    )
    parser.add_argument(
        "--long-track-length",
        type=int,
        help="Track length counted as a long false track.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Do not print artifact paths and summary metrics as JSON.",
    )
    return parser
            ...
            json.dumps(
                {
                    ...
                    "false_accepted_tracks": metrics["velocities"][
...
def _build_config(
    ...
    mode = args.mode or _string_option(
        output_config, "detection_mode", ("detection", "mode"), "positive"
    )
    low_threshold = _float_option(
        args.low_threshold,
        output_config,
        "detection_low_threshold",
        ("detection", "low_threshold"),
        0.0,
    )
    low_threshold = (
        None if low_threshold is None or low_threshold == 0 else low_threshold
    )
    ...
    return MapOnlyNegativeControlConfig(
        ...
        min_area_px=_int_option(
            args.min_area_px,
            output_config,
            "min_area_px",
            ("detection", "min_area_px"),
            4,
        ),
        max_area_px=_none_if_nonpositive_int(
            _int_option(
                args.max_area_px,
                output_config,
                "detection_max_area_px",
                ("detection", "max_area_px"),
                0,
            )
        ),
        min_bbox_width_px=_none_if_nonpositive_int(
            _int_option(
                args.min_bbox_width_px,
                output_config,
                "detection_min_bbox_width_px",
                ("detection", "min_bbox_width_px"),
                0,
            )
        ),
        min_bbox_height_px=_none_if_nonpositive_int(
            _int_option(
                args.min_bbox_height_px,
                output_config,
                "detection_min_bbox_height_px",
                ("detection", "min_bbox_height_px"),
                0,
            )
        ),
        max_bbox_aspect_ratio=_none_if_nonpositive_float(
            _float_option(
                args.max_bbox_aspect_ratio,
                output_config,
                "detection_max_bbox_aspect_ratio",
                ("detection", "max_bbox_aspect_ratio"),
                0.0,
            )
        ),
        min_bbox_extent=_none_if_nonpositive_float(
            _float_option(
                args.min_bbox_extent,
                output_config,
                "detection_min_bbox_extent",
                ("detection", "min_bbox_extent"),
                0.0,
            )
        ),
        split_merged_components=_bool_option(
            args.split_merged_components,
            output_config,
            "detection_split_merged_components",
            ("detection", "split_merged_components"),
            False,
        ),
        split_min_projection_gap_px=_int_option(
            args.split_min_projection_gap_px,
            output_config,
            "detection_split_min_projection_gap_px",
            ("detection", "split_min_projection_gap_px"),
            2,
        ),
        ...
        frame_count=(
            None
            if args.frame_count is None or args.frame_count == 0
            else args.frame_count
        ),
        belt_velocity_px_per_frame=_belt_velocity_from_defaults(
            args, output_config, metadata
        ),
        ...
        tracking_max_frame_gap=(
            args.tracking_max_frame_gap
            if args.tracking_max_frame_gap is not None
            else _float_option(
                None,
                output_config,
                "tracking_max_frame_gap",
        ...
        or _string_option(
            ...
            ("tracking", "velocity_fit_method"),
            "linear",
        ),
        track_filter_min_length=(
            ...
            if args.track_filter_min_length is not None
                ...
                output_config,
                "track_filter_min_length",
                ("track_filter", "min_length"),
                5,
            )
        ),
        track_filter_min_velocity_ratio_y=(
            args.track_filter_min_velocity_ratio_y
            if args.track_filter_min_velocity_ratio_y is not None
            else _float_option(
                None,
                output_config,
                "track_filter_min_velocity_ratio_y",
                ("track_filter", "min_velocity_ratio_y"),
                0.0,
            )
        ),
        track_filter_max_velocity_ratio_y=(
            args.track_filter_max_velocity_ratio_y
            if args.track_filter_max_velocity_ratio_y is not None
            else _float_option(
                None,
                output_config,
                "track_filter_max_velocity_ratio_y",
                ("track_filter", "max_velocity_ratio_y"),
                1.1,
            )
        ),
        track_filter_max_abs_x_velocity_px_per_frame=_none_if_nonpositive_float(
            ...
            if args.track_filter_max_abs_x_velocity_px_per_frame is not None
                ...
                ("track_filter", "max_abs_x_velocity_px_per_frame"),
                0.0,
            )
        ),
        long_track_length=(
            args.long_track_length if args.long_track_length is not None else 10
        ),
    )
...
def _config_value(
    config: dict[str, Any], flat_name: str, nested_path: tuple[str, ...]
) -> Any:
...
def _belt_velocity_from_defaults(
    ...
    for key in (
        ...
        if parsed is not None:
            return parsed
    value = _config_value(
        config, "belt_velocity_px_per_frame", ("belt", "velocity_px_per_frame")
    )
...
def _period_from_defaults(
    ...
    parsed = _finite_float(
        _config_value(config, "belt_period_px", ("belt", "period_px"))
    )
