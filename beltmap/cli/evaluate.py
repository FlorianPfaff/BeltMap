...
def parse_run_spec(value: str) -> RunSpec:
    if "=" in value:
        ...
        if not path_text:
            raise argparse.ArgumentTypeError("run output directory after '=' must not be empty")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Compare BeltMap output directories and write JSON/CSV/Markdown ablation summaries.",
    )
    parser.add_argument(
        "--run",
        action="append",
        ...
        help=(
            ...
            "Use NAME=PATH to give stable ablation labels; a bare path uses its directory name."
        ),
    )
    parser.add_argument(
        "--output-dir",
