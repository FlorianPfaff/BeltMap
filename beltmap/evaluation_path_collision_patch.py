...
def _unwrap_patched_callable(func: Any) -> Any:
    """Return the original evaluation writer behind this patch."""
    ...
    *,
    ...
    return {
        ...
        "markdown": (
            ...
            else output_dir / "evaluation_summary.md"
        ),
    }


def _validate_output_paths(
    runs: list[_evaluation.RunSpec],
    output_paths: dict[str, Path],
) -> None:
    for (first_name, first_path), (second_name, second_path) in combinations(
        output_paths.items(), 2
    ):
        ...
        for artifact_name in _evaluation.STANDARD_OUTPUT_FILES:
            ...
            for output_name, output_path in output_paths.items():
                if _paths_refer_to_same_file(output_path, artifact_path):
                    raise ValueError(
                        f"evaluation {output_name} output must not overwrite "
                        f"run artifact {artifact_path}"
                    )
...
def write_evaluation_without_path_collisions(
    ...
    *,
    ...
    """Write evaluation artifacts only after validating every destination path."""
...
setattr(
    write_evaluation_without_path_collisions,
    _ORIGINAL_ATTR,
    _original_write_evaluation,
)
