"""Ignore directory entries during comparison preview discovery.

``Path.glob`` yields matching directories as well as regular files.  A directory
whose name looks like ``raw_frame_000001.png`` or
``residual_fixed_frame_000001.png`` must not be handed to Pillow as a saved
preview image.
"""
...
_PATCHED_ATTR = "_beltmap_compare_preview_file_patched"
_ORIGINAL_ATTR = "_beltmap_original_find_named_preview_paths"
...
def find_named_preview_files(output_dir: Path, prefix: str) -> dict[int, Path]:
    """Return only regular preview files keyed by parsed frame index."""
...
setattr(find_named_preview_files, _PATCHED_ATTR, True)
setattr(
    find_named_preview_files,
    _ORIGINAL_ATTR,
    _original_find_named_preview_paths,
)
_compare_runs.find_named_preview_paths = find_named_preview_files
