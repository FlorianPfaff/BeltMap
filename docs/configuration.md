...
| `detection.mask_mode` | `DETECTION_MASK_MODE` | `--detection-mask-mode` | `threshold` | mode | Final mask mode. `threshold` preserves legacy behavior; `hysteresis` keeps weak support pixels only when connected to a strong seed above `detection.threshold`. |
| `detection.grow_threshold` | `DETECTION_GROW_THRESHOLD` | `--detection-grow-threshold` | `0` | z | Weak support threshold for `detection.mask_mode = "hysteresis"`. `0` auto-selects half of `detection.threshold`. |
| `detection.max_moment_aspect_ratio` | `DETECTION_MAX_MOMENT_ASPECT_RATIO` | `--detection-max-moment-aspect-ratio` | `0` | ratio | Optional maximum second-moment aspect ratio. This catches line-like scratches even when their bounding box is not extreme. `0` disables this gate. |
| `detection.min_compactness` | `DETECTION_MIN_COMPACTNESS` | `--detection-min-compactness` | `0` | fraction | Optional minimum compactness `4*pi*area/perimeter^2`. Low values reject thin scratches and hollow fragments. `0` disables this gate. |
| `detection.min_border_margin_px` | `DETECTION_MIN_BORDER_MARGIN_PX` | `--detection-min-border-margin-px` | `0` | px | Optional minimum distance from a component bounding box to the crop border. `0` disables this gate. |
| `detection.min_mean_signal` | `DETECTION_MIN_MEAN_SIGNAL` | `--detection-min-mean-signal` | `0` | z | Optional minimum mean normalized residual over the component. `0` disables this gate. |
| `detection.min_peak_signal` | `DETECTION_MIN_PEAK_SIGNAL` | `--detection-min-peak-signal` | `0` | z | Optional minimum peak normalized residual over the component. `0` disables this gate. |
| `detection.signal_core_threshold` | `DETECTION_SIGNAL_CORE_THRESHOLD` | `--detection-signal-core-threshold` | `0` | z | Threshold used to count high-confidence core pixels inside each component. Required when core gates are enabled. |
| `detection.min_core_area_px` | `DETECTION_MIN_CORE_AREA_PX` | `--detection-min-core-area-px` | `0` | px | Optional minimum number of component pixels above `detection.signal_core_threshold`. `0` disables this gate. |
| `detection.min_core_fraction` | `DETECTION_MIN_CORE_FRACTION` | `--detection-min-core-fraction` | `0` | fraction | Optional minimum fraction of component pixels above `detection.signal_core_threshold`. `0` disables this gate. |
| `detection.local_contrast_margin_px` | `DETECTION_LOCAL_CONTRAST_MARGIN_PX` | `--detection-local-contrast-margin-px` | `4` | px | Width of the annulus around the component bounding box used for local-background contrast diagnostics. |
| `detection.min_mean_local_contrast` | `DETECTION_MIN_MEAN_LOCAL_CONTRAST` | `--detection-min-mean-local-contrast` | `0` | z | Optional minimum difference between component mean residual and local background. `0` disables this gate. |
| `detection.min_peak_local_contrast` | `DETECTION_MIN_PEAK_LOCAL_CONTRAST` | `--detection-min-peak-local-contrast` | `0` | z | Optional minimum difference between component peak residual and local background. `0` disables this gate. |
| `detection.mask_closing_radius_px` | `DETECTION_MASK_CLOSING_RADIUS_PX` | `--detection-mask-closing-radius-px` | `0` | px | Optional binary closing radius applied to the threshold mask before components are extracted. Useful for static-background runs with fragmented bright particles. |
| `detection.mask_fill_holes` | `DETECTION_MASK_FILL_HOLES` | `--detection-mask-fill-holes` / `--no-detection-mask-fill-holes` | `false` | bool | Fill enclosed holes in the threshold mask before component extraction. |
| `detection.mask_opening_radius_px` | `DETECTION_MASK_OPENING_RADIUS_PX` | `--detection-mask-opening-radius-px` | `0` | px | Optional binary opening radius applied after closing/fill-hole cleanup. Use cautiously; it can erode small true particles. |
| `detection.mask_min_area_px` | `DETECTION_MASK_MIN_AREA_PX` | `--detection-mask-min-area-px` | `0` | px | Optional minimum threshold-mask component area after morphology. `0` disables this pre-component area cleanup. |
| `diagnostics.write_candidate_detections` | `WRITE_CANDIDATE_DETECTIONS` | `--write-candidate-detections` / `--no-write-candidate-detections` | `false` | bool | Write `candidate_detections.csv` with one row per post-mask component, accepted/rejected status, and rejection reasons from the configured component gates. |
...
| `static_background.noise_retain_fraction` | `STATIC_BACKGROUND_NOISE_RETAIN_FRACTION` | `--static-background-noise-retain-fraction` | `0` | fraction | Keep this fraction of the pre-static-background local-noise estimate as a floor after subtracting the learned additive background. This helps prevent overconfident normalization on textured belt regions. |
| `static_background.min_corrected_noise` | `STATIC_BACKGROUND_MIN_CORRECTED_NOISE` | `--static-background-min-corrected-noise` | `0` | gray | Absolute minimum corrected local-noise floor after static-background subtraction. |
| `residual_correction.bias_mode` | `RESIDUAL_BIAS_CORRECTION_MODE` | `--residual-bias-correction-mode` | `none` | mode | Optional per-frame additive residual-bias correction after static-background subtraction and before static-noise flooring. `median` removes a scalar frame offset; `row_median` removes row-wise offsets such as line-light drift. |
| `residual_correction.bias_mask_threshold` | `RESIDUAL_BIAS_MASK_THRESHOLD` | `--residual-bias-mask-threshold` | `0` | z | Optional normalized residual threshold used to exclude bright particle pixels from the per-frame bias estimate. Values near the detection threshold are a good starting point. `0` disables this mask. |
| `residual_correction.bias_row_smoothing_window_px` | `RESIDUAL_BIAS_ROW_SMOOTHING_WINDOW_PX` | `--residual-bias-row-smoothing-window-px` | `31` | px | Moving-average smoothing window for row-wise bias estimates. Use `0` or `1` to disable smoothing. |
...
| `recurrent_artifact.soft_signal` | `RECURRENT_ARTIFACT_SOFT_SIGNAL` | `--recurrent-artifact-soft-signal` | `peak_signal` | mode | Signal used by soft recurrent filtering: `peak_signal`, `peak_local_contrast`, or `peak_and_local_contrast`. |
