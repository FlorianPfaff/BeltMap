...
| `reuse.map_support_path` | `REUSE_MAP_SUPPORT_PATH` | `--reuse-map-support-path` | unset; auto-detects sibling `belt_map_support.npy` when reusing `belt_map.npy` | path | Existing `belt_map_support.npy` to reuse for belt-coordinate support/risk diagnostics and optional map-risk detection filtering. |
...
| `detection.local_illumination_correction` | `DETECTION_LOCAL_ILLUMINATION_CORRECTION` | `--detection-local-illumination-correction` / `--no-detection-local-illumination-correction` | `false` | bool | Experimental ablation: before final detection, fit a low-frequency additive residual field from particle-masked tile medians and add it to the expected background. Disabled by default; writes `local_illumination_fits.csv` when enabled. |
| `detection.local_illumination_tile_px` | `DETECTION_LOCAL_ILLUMINATION_TILE_PX` | `--detection-local-illumination-tile-px` | `64` | px | Tile size for the low-frequency local illumination field. Larger values make the correction smoother and less able to absorb particles. |
| `detection.local_illumination_min_pixels` | `DETECTION_LOCAL_ILLUMINATION_MIN_PIXELS` | `--detection-local-illumination-min-pixels` | `128` | pixels | Minimum number of non-particle residual pixels required to fit and apply the local illumination field for one frame. |
| `detection.local_illumination_mask_threshold` | `DETECTION_LOCAL_ILLUMINATION_MASK_THRESHOLD` | `--detection-local-illumination-mask-threshold` | follows `map.particle_mask_threshold` | z | Strong threshold used to mask particle-like residuals before fitting the local illumination field. Leave unset for the map-mask default; set explicitly for ablations. |
| `detection.local_illumination_mask_mode` | `DETECTION_LOCAL_ILLUMINATION_MASK_MODE` | `--detection-local-illumination-mask-mode` | follows `map.particle_mask_mode` | mode | Particle-mask mode for local illumination fitting. Valid values match `map.particle_mask_mode`: `positive`, `negative`, `absolute`, and `hysteresis_abs`. |
| `detection.local_illumination_mask_grow_threshold` | `DETECTION_LOCAL_ILLUMINATION_MASK_GROW_THRESHOLD` | `--detection-local-illumination-mask-grow-threshold` | follows `map.particle_mask_grow_threshold` | z | Lower hysteresis threshold for `hysteresis_abs` local-illumination masks. |
| `detection.local_illumination_mask_dilation_px` | `DETECTION_LOCAL_ILLUMINATION_MASK_DILATION_PX` | `--detection-local-illumination-mask-dilation-px` | follows `map.particle_mask_dilation_px` | px | Morphological dilation radius applied to local-illumination particle masks. |
| `detection.local_illumination_mask_margin_px` | `DETECTION_LOCAL_ILLUMINATION_MASK_MARGIN_PX` | `--detection-local-illumination-mask-margin-px` | follows `map.particle_mask_margin_px` | px | Safety margin around particle boxes while fitting the local illumination field. |
| `detection.local_illumination_mask_min_area_px` | `DETECTION_LOCAL_ILLUMINATION_MASK_MIN_AREA_PX` | `--detection-local-illumination-mask-min-area-px` | follows `map.particle_mask_min_area_px` | px | Minimum component area for local-illumination particle masks. |
...
| `tracking.max_match_distance_px` | `MAX_MATCH_DISTANCE_PX` | `--max-match-distance-px` | `max(5, 1.5 * abs(belt_velocity))` | px | Maximum frame-to-frame association distance for PyRecEst tracking. Leave unset to derive it from the belt speed. |
...
| `track_filter.max_recurrent_artifact_track_score` | `TRACK_FILTER_MAX_RECURRENT_ARTIFACT_SCORE` | `--track-filter-max-recurrent-artifact-track-score` | `0` | fraction | Optional track-level recurrent-artifact gate. `0` disables this gate. Values near `1` only reject tracks whose detections repeatedly overlap recurrent belt-coordinate artifact evidence. |
| `track_filter.recurrent_artifact_detection_threshold` | `TRACK_FILTER_RECURRENT_ARTIFACT_DETECTION_THRESHOLD` | `--track-filter-recurrent-artifact-detection-threshold` | `0.3` | fraction | Per-detection recurrent-artifact evidence threshold used to compute the track-level hit fraction. The evidence is the larger of the stored detection overlap fraction and recurrent-artifact probability. |
...
| `map.reconstruction_trim_fraction` | `MAP_RECONSTRUCTION_TRIM_FRACTION` | `--map-reconstruction-trim-fraction` | `0`; `0.1` for `trimmed_mean`/`winsorized_mean` | fraction | Symmetric per-pixel trim/winsorization fraction for robust belt-map reconstruction. Must be in `[0, 0.5)`. |
...
| `map.frame_median_offset_correction` | `MAP_FRAME_MEDIAN_OFFSET_CORRECTION` | `--map-frame-median-offset-correction` / `--no-map-frame-median-offset-correction` | `false` | bool | Normalize sampled frames with a robust median brightness offset before belt-map accumulation. Initial passes use sampled frame medians; refinement passes with a provisional map use the frame-minus-expected residual median. This targets global additive illumination drift during map reconstruction. |
| `map.aggregation` | `MAP_AGGREGATION` | `--map-aggregation` | `mean` | method | Belt-map aggregation method. `mean` is the streaming arithmetic mean. `trimmed_mean` computes a per-belt-pixel trimmed mean from sampled observations. `winsorized_mean` clips per-pixel outliers before averaging. `huber` performs additional robust passes that downweight particle-like outliers missed by the mask. |
| `map.robust_iterations` | `MAP_ROBUST_ITERATIONS` | `--map-robust-iterations` | `1` | passes | Number of Huber aggregation refinement passes when `map.aggregation = "huber"`. |
| `map.robust_huber_delta` | `MAP_ROBUST_HUBER_DELTA` | `--map-robust-huber-delta` | `3.0` | sigma | Huber cutoff in robust residual-scale units for robust map aggregation. |
| `map.robust_min_scale` | `MAP_ROBUST_MIN_SCALE` | `--map-robust-min-scale` | `1.0` | gray | Minimum residual scale used by robust Huber map aggregation. |
| `map_risk.min_support` | `MAP_RISK_MIN_SUPPORT` | `--map-risk-min-support` | `1.0` | effective observations | Support below this value is treated as low support in `belt_map_low_support_mask.npy`; interpolated pixels have risk 1. |
| `map_risk.reject_max_mean` | `MAP_RISK_REJECT_MAX_MEAN` | `--map-risk-reject-max-mean` | `1.0` | fraction | Optional detection gate: reject components whose bbox mean map risk exceeds this value. `1.0` disables the gate. |
...
| `revolution_split.enabled` | `REVOLUTION_SPLIT_ENABLED` | `--revolution-split-enabled` / `--no-revolution-split-enabled` | `false` | bool | Build the belt map only from train revolutions and hold out eval revolutions for ghost diagnostics. Requires `belt.period_px` / `BELT_PERIOD_PX` and cannot be combined with `reuse.belt_map_path`. |
| `revolution_split.eval_every` | `REVOLUTION_SPLIT_EVAL_EVERY` | `--revolution-split-eval-every` | `3` | revolutions | Hold out every Nth observed revolution when `revolution_split.eval_revolutions` is unset. |
| `revolution_split.eval_offset` | `REVOLUTION_SPLIT_EVAL_OFFSET` | `--revolution-split-eval-offset` | `0` | revolutions | Modulo offset used with `revolution_split.eval_every`. With `eval_every = 3` and `eval_offset = 1`, revolutions 1, 4, 7, ... are held out. |
| `revolution_split.eval_revolutions` | `REVOLUTION_SPLIT_EVAL_REVOLUTIONS` | `--revolution-split-eval-revolutions` | unset | list | Optional comma-separated explicit held-out revolutions or closed ranges, for example `1,4,7-9`. Overrides `eval_every` when set. |
| `revolution_split.min_train_revolutions` | `REVOLUTION_SPLIT_MIN_TRAIN_REVOLUTIONS` | `--revolution-split-min-train-revolutions` | `1` | revolutions | Minimum number of train revolutions required after splitting. |
| `revolution_split.min_eval_revolutions` | `REVOLUTION_SPLIT_MIN_EVAL_REVOLUTIONS` | `--revolution-split-min-eval-revolutions` | `1` | revolutions | Minimum number of held-out eval revolutions required after splitting. |
| `revolution_split.ghost_min_revolutions` | `REVOLUTION_SPLIT_GHOST_MIN_REVOLUTIONS` | `--revolution-split-ghost-min-revolutions` | `2` | revolutions | Minimum train revolutions required to mark the train-only artifact map used to score held-out ghost candidates. |
...
| `registration_quality.enabled` | `REGISTRATION_QUALITY_ENABLED` | `--registration-quality-enabled` / `--no-registration-quality-enabled` | `false` | bool | Enable frame-level phase-registration quality gates after residual rendering and before final detection. |
| `registration_quality.action` | `REGISTRATION_QUALITY_ACTION` | `--registration-quality-action` | `report` | mode | Response for low-quality registration frames: `report` writes diagnostics only, `inflate` increases residual noise before detection, and `skip` writes no detections for the frame. |
| `registration_quality.min_score` | `REGISTRATION_QUALITY_MIN_SCORE` | `--registration-quality-min-score` | `0` | score | Minimum registration score accepted without quality action. `0` disables this gate. |
| `registration_quality.min_loss_gap_ratio` | `REGISTRATION_QUALITY_MIN_LOSS_GAP_RATIO` | `--registration-quality-min-loss-gap-ratio` | `0` | ratio | Minimum relative loss gap between the best and second-best phase-registration candidate. `0` disables this gate. |
| `registration_quality.max_uncertainty_px` | `REGISTRATION_QUALITY_MAX_UNCERTAINTY_PX` | `--registration-quality-max-uncertainty-px` | `0` | px | Maximum curvature-derived registration uncertainty accepted without quality action. `0` disables this gate. |
| `registration_quality.max_abs_correction_px` | `REGISTRATION_QUALITY_MAX_ABS_CORRECTION_PX` | `--registration-quality-max-abs-correction-px` | `0` | px | Maximum absolute phase correction accepted without quality action. `0` disables this gate. |
| `registration_quality.noise_inflation_factor` | `REGISTRATION_QUALITY_NOISE_INFLATION_FACTOR` | `--registration-quality-noise-inflation-factor` | `2.0` | factor | Base residual-noise multiplier used by `registration_quality.action = "inflate"`. Must be at least 1. |
| `registration_quality.uncertainty_inflation_scale` | `REGISTRATION_QUALITY_UNCERTAINTY_INFLATION_SCALE` | `--registration-quality-uncertainty-inflation-scale` | `0` | factor/px | Optional extra noise multiplier per uncertainty pixel for `inflate` mode. The applied multiplier is at least `noise_inflation_factor`. |
