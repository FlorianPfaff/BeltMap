...
| `map.aggregation` | `MAP_AGGREGATION` | `--map-aggregation` | `mean` | method | Belt-map aggregation method. `mean` is the streaming arithmetic mean. `huber` performs additional robust passes that downweight particle-like outliers missed by the mask. |
