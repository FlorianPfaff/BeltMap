...
Frame-to-frame association uses PyRecEst global nearest-neighbor multitarget tracking with a maximum match distance. New tracks start from the configured velocity prior, and established tracks are predicted from a robust recent trajectory estimate to reduce fragmentation when particles drift away from the nominal belt-speed prior. Prediction residuals set a bounded per-track covariance for PyRecEst's Mahalanobis distance, so stable tracks remain sharp while noisy tracks are treated as less certain inside the same hard pixel gate. This covariance also scales with the frame gap being bridged, so an otherwise plausible track is not over-penalized after a missed detection. PyRecEst also receives conservative pairwise area/signal consistency costs from recent track medians so ambiguous associations prefer detections that still look like the same particle without overreacting to one noisy component. The pairwise costs are capped and outside-distance pairs are blocked so appearance costs cannot silently shrink or expand the configured spatial gate. Per-track particle velocity is then estimated by a linear slope of centroid position versus frame index. Belt-relative output includes
