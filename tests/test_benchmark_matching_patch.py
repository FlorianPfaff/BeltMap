...
def test_benchmark_and_bootstrap_maximize_valid_match_cardinality(
    monkeypatch,
) -> None:
    truth = {
        "particles": [
    ...
    bootstrap_metrics = bootstrap_ci.aggregate_labeled_outcomes(outcomes)
    assert bootstrap_metrics["labeled_true_positives"] == 2
    assert bootstrap_metrics["labeled_false_positives"] == 0
    assert bootstrap_metrics["labeled_false_negatives"] == 0
    assert bootstrap_metrics["labeled_precision"] == 1.0
    assert bootstrap_metrics["labeled_recall"] == 1.0
    assert bootstrap_metrics["labeled_f1"] == 1.0
    assert bootstrap_metrics["labeled_mean_matched_iou"] == pytest.approx(
        (0.80 + 0.85) / 2.0
    )
