...
def test_phase_estimate_row_omits_cycle_fields_for_inferred_finite_support(monkeypatch):
...
def test_phase_estimate_row_preserves_cycle_fields_for_known_period(monkeypatch):
    monkeypatch.setenv("BELT_PERIOD_PX", "123")
    monkeypatch.delenv("REUSE_BELT_MAP_PATH", raising=False)
    seen: dict[str, float] = {}
    monkeypatch.setattr(patch, "_original_phase_estimate_row", _stub_phase_estimate_row(seen))
...
def test_texture_phase_velocity_skips_periodic_summary_for_unknown_period(monkeypatch):
    monkeypatch.delenv("BELT_PERIOD_PX", raising=False)
    monkeypatch.delenv("REUSE_BELT_MAP_PATH", raising=False)
    called = False
