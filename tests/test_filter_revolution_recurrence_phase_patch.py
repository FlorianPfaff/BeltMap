...
@pytest.mark.parametrize("frame_index", ["0.5", "nan", "inf", "bad"])
def test_phase_loader_rejects_invalid_frame_identifiers(tmp_path, frame_index):
    phase_path = tmp_path / "phase_estimates.csv"
    write_phase_rows(phase_path, [{"frame_index": frame_index, "phase_px": 1.0}])
    with pytest.raises(ValueError, match="invalid frame_index"):
        cli_filter.load_phase_px_by_frame(phase_path, frame_count=2)


@pytest.mark.parametrize("phase_px", ["nan", "inf", "bad", ""])
def test_phase_loader_rejects_invalid_phases(tmp_path, phase_px):
    phase_path = tmp_path / "phase_estimates.csv"
    write_phase_rows(phase_path, [{"frame_index": 0, "phase_px": phase_px}])
